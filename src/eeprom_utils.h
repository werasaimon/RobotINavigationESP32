...
void saveCalibrationFix()
{
    Serial.println("Write calibrated parameters to EEPROM");

    float accel_bias[3] = {-23.34, -82.83, -7.57};
    float gyro_bias[3] = { 0.06, 0.04, -0.37 };
    float mag_bias[3] = {-7.10, 346.56, 118.41};
    float mag_scale[3] = {1.00, 1.02, 0.98};

    writeByte(EEP_CALIB_FLAG, 1);
    writeFloat(EEP_ACC_BIAS + 0, accel_bias[0]);
    writeFloat(EEP_ACC_BIAS + 4, accel_bias[1]);
    writeFloat(EEP_ACC_BIAS + 8, accel_bias[2]);
    writeFloat(EEP_GYRO_BIAS + 0, gyro_bias[0]);
    writeFloat(EEP_GYRO_BIAS + 4, gyro_bias[1]);
    writeFloat(EEP_GYRO_BIAS + 8, gyro_bias[2]);
    writeFloat(EEP_MAG_BIAS + 0, mag_bias[0]);
    writeFloat(EEP_MAG_BIAS + 4, mag_bias[1]);
    writeFloat(EEP_MAG_BIAS + 8, mag_bias[2]);
    writeFloat(EEP_MAG_SCALE + 0, mag_scale[0]);
    writeFloat(EEP_MAG_SCALE + 4, mag_scale[1]);
    writeFloat(EEP_MAG_SCALE + 8, mag_scale[2]);
#if defined(ESP_PLATFORM) || defined(ESP8266)
    EEPROM.commit();
#endif
}
